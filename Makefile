...
# `make nightly` :  dev keyring, fedora-nightlies rpm via qubes-dom0-update
# `make staging` :  staging keyring, staging rpm via qubes-dom0-update
...
dev staging nightly: assert-dom0 ## Configure environment
	...
	$(MAKE) install-rpm RPM_INSTALL_STRATEGY=$(RPM_STRATEGY)
