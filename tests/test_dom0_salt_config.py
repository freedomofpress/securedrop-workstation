...
def test_trust_desktop_launcher(run_as_gui_user):
    # salt should have already provisioned the correct checksum
    res = subprocess.run(
        run_as_gui_user + ["/usr/bin/securedrop/sdw-desktop-trust", "--check"], check=False
    )
    ...
    res = subprocess.run(
        run_as_gui_user + ["/usr/bin/securedrop/sdw-desktop-trust"],
        text=True,
        check=False,
        capture_output=True,
    )
